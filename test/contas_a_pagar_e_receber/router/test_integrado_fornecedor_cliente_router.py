...
def test_deve_listar_fornecedor_cliente():
    ...
    assert response.json() == [{"id": 1, "nome": "CPFL"}, {"id": 2, "nome": "Sanasa"}]
...
def test_deve_atualizar_fornecedor_cliente():
    ...
    response = client.post("/fornecedor-cliente", json={"nome": "Extra Super Mercados"})

    id_do_fornecedor_cliente = response.json()["id"]

    response_put = client.put(
        f"/fornecedor-cliente/{id_do_fornecedor_cliente}", json={"nome": "Giga Extra"}
    )
...
def test_deve_retornar_nao_encontrado_para_id_nao_existente_na_atualizacao():
    ...
    response_put = client.put("/fornecedor-cliente/100", json={"nome": "Giga Extra"})
...
def test_deve_remover_fornecedor_cliente():
    ...
    response = client.post("/fornecedor-cliente", json={"nome": "Adde Sistemas"})
...
def test_deve_retornar_erro_quando_o_nome_nao_estiver_dentro_dos_limites():
    response = client.post("/fornecedor-cliente", json={"nome": "AS"})
